...
export function PeopleTab({
  ...
  const handleEditUser = async (data) => {
    ...
    try {
      if (data.role !== editingUser.role) {
        ...
        const confirmed = sensitiveActionPolicy?.protectRoleChanges === false || sensitiveActionPolicy?.requireConfirmation === false
          || window.confirm(`Apply ${data.role} role to this user?`);
        if (!confirmed) return;
        ...
        if (sensitiveActionPolicy?.protectRoleChanges !== false && sensitiveActionPolicy?.requireAdminReason && !reason) {
          throw new Error("A change reason is required by workspace policy.");
        }
        if (e2eMode) updateE2EAuthUserRole(editingUser.id, data.role);
        else await changeWorkspaceUserRole(editingUser.id, data.role, reason);
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {showUserForm && !editingUser && canPerform("user:invite") && <UserForm busy={userAction === "invite"} onSave={handleInviteUser} onCancel={() => setShowUserForm(false)} />}
      {editingUser && canPerform("user:manage") && <UserForm initial={editingUser} busy={userAction === editingUser.id} lockRole={editingUser.id === currentUid || !canPerform("role:manage")} onSave={handleEditUser} onCancel={() => setEditingUser(null)} />}

      {!showUserForm && !editingUser && canPerform("user:invite") && (
        <button
              ...
              <div className="flex items-start gap-4 p-4">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {userTeams.map((team) => (
                      <span key={team.id} className="group flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border font-medium transition-all" style={{ backgroundColor: team.color + "15", borderColor: team.color + "50", color: team.color }}>
                        ...
                        {team.name}
                        {canPerform("team:manage") && <button className="opacity-0 group-hover:opacity-100 ml-0.5 hover:text-red-500 transition-all leading-none" onClick={() => removeFromTeam(team.id, user.username)} title={`Remove from ${team.name}`}>
                          <FaTimes className="w-2.5 h-2.5" />
                        </button>}
                      </span>
                    ))}

                    {canPerform("team:manage") && availableTeams.length > 0 && (
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
                    ...
                    {userProjects.map((project) => (
                      <span key={project.id} className="group flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border font-medium transition-all" style={{ backgroundColor: project.color + "15", borderColor: project.color + "50", color: project.color }}>
                        ...
                        {project.name}
                        {canPerform("project:manage") && <button className="opacity-0 group-hover:opacity-100 ml-0.5 hover:text-red-500 transition-all leading-none" onClick={() => removeFromProject(project.id, user.username)} title={`Remove from ${project.name}`}>
                          <FaTimes className="w-2.5 h-2.5" />
                        </button>}
                      </span>
                    ))}
                    {canPerform("project:manage") && availableProjects.length > 0 && (
                      <div className="relative">
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canPerform("user:manage") && <button onClick={() => { setEditingUser(user); setShowUserForm(false); }} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Edit user">
                      <FaEdit className="w-3.5 h-3.5" />
                    </button>}
                    {canPerform("user:manage") && (deletingUserId === user.id ? (
                      <DeleteConfirm
                        label="Remove?"
                    ...
                    {canPerform("user:manage") && <button disabled={user.id === currentUid || userAction === user.id} onClick={() => handleStatusChange(user)} className={`text-xs px-2.5 py-1 rounded-lg border font-medium transition-all disabled:cursor-not-allowed disabled:opacity-40 ${
                      user.status === "active"
                      ...
                      <div className="space-y-1 max-h-40 overflow-y-auto pr-0.5">
                        {userTasks.map((task) => (
                          <div key={task.id} className="flex items-center gap-2 bg-white dark:bg-[#1c2030] border border-slate-100 dark:border-[#2a3044] rounded-lg px-2.5 py-1.5">
                            ...
                            {canPerform("task:edit") && <button onClick={() => unassignTask(task.id)} className="flex-shrink-0 text-slate-300 hover:text-red-400 dark:text-slate-600 dark:hover:text-red-400 transition-colors ml-1" title="Unassign">
                              <FaTimes className="w-2.5 h-2.5" />
                            </button>}
                          </div>
                        ))}
