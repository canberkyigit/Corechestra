...
export default function ProjectSettingsModal({ project, onClose }) {
  ...
  const [workflowRules, setWorkflowRules] = useState({
    requireReviewBeforeDone: workspaceDefaultWorkflow.requireReviewBeforeDone || false,
    captureBlockReason: workspaceDefaultWorkflow.captureBlockReason !== false,
    notifyOnBlocked: workspaceDefaultWorkflow.notifyOnBlocked !== false,
    allowBackwardMoves: workspaceDefaultWorkflow.allowBackwardMoves !== false,
  ...
  const [sprintTemplateId, setSprintTemplateId] = useState(project.sprintDefaults?.templateId || workspaceDefaultTemplates.sprint || templateRegistry?.sprint?.[0]?.id || "");
  const [templateDefaults, setTemplateDefaults] = useState({
    release: project.templateDefaults?.release || workspaceDefaultTemplates.release || templateRegistry?.release?.[0]?.id || "",
    onboarding: project.templateDefaults?.onboarding || workspaceDefaultTemplates.onboarding || templateRegistry?.onboarding?.[0]?.id || "",
    approval: project.templateDefaults?.approval || workspaceDefaultTemplates.approval || templateRegistry?.approval?.[0]?.id || "",
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            {tab === "workflow" && (
              <div className="space-y-4">
                ...
                <div className="rounded-xl border border-slate-200 dark:border-[#2a3044] bg-slate-50 dark:bg-[#141720] p-4 space-y-3">
                  <div>
                    <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Workflow Rules</h4>
                    <p className="text-xs text-slate-400 mt-1">Set project-specific transition expectations without changing the task model.</p>
                  </div>
                  {[
                    ["requireReviewBeforeDone", "Require Review before Done", "Encourage teams to move work through review before completion."],
                    ["captureBlockReason", "Require blocker reason", "When a task becomes blocked, teams should document why."],
                    ["notifyOnBlocked", "Notify when blocked", "Send stronger signals when work is blocked."],
                    ["allowBackwardMoves", "Allow backward transitions", "Permit moving tasks back to earlier statuses when needed."],
                    ...
                    <div key={key} className={cardCls}>
                      ...
                      <Toggle on={workflowRules[key]} onToggle={() => setWorkflowRules((prev) => ({ ...prev, [key]: !prev[key] }))} />
                    </div>
                  ))}
                </div>
            ...
            {tab === "sprint" && (
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Sprint Template</label>
                  <select value={sprintTemplateId} onChange={(e) => setSprintTemplateId(e.target.value)} className={inputCls}>
                    {(templateRegistry?.sprint || []).map((template) => (
                      <option key={template.id} value={template.id}>{template.name}</option>
                    ))}
                  </select>
                  <p className="text-[11px] text-slate-400 mt-1">
                    New sprint planning can inherit prompts and checklists from this template.
                  </p>
                </div>

                <div className="rounded-xl border border-slate-200 dark:border-[#2a3044] bg-slate-50 dark:bg-[#141720] p-4 space-y-3">
                  <div>
                    <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Template Defaults</h4>
                    <p className="text-xs text-slate-400 mt-1">Choose default templates this project should use across release, HR and incident-related flows.</p>
                  </div>
                  {[
