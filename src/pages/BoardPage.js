...
import FuturePlansModal from "../components/FuturePlansModal";
...
import AllSprintsTab from "../components/tabs/AllSprintsTab";
...
function ListView({ tasks, onTaskClick, selectedIds, onToggleSelect, bulkMode, onCreateTask }) {
  ...
  // Flatten groups into virtual items: header | task | spacer
  const flatItems = useMemo(() => {
    const items = [];
    Object.entries(grouped).forEach(([status, grpTasks]) => {
      items.push({ kind: "header", status, count: grpTasks.length });
      grpTasks.forEach((task) => items.push({ kind: "task", task }));
    ...
    return (
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          {onCreateTask && (
            <button className="mt-4 px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors" onClick={onCreateTask}>
              Create Task
            </button>
          )}
        </div>
...
const COL_WIDTHS = { check: 36, title: "1fr", status: 110, priority: 90, assignee: 110, due: 110, points: 60 };
...
export default function BoardPage({ forcedTab, onForcedTabConsumed }) {
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50 dark:bg-[#141720] flex flex-col transition-colors">
      ...
      {/* Future Plans Modal */}
      <FuturePlansModal
        open={futurePlansOpen}
        onClose={() => setFuturePlansOpen(false)}
      />
